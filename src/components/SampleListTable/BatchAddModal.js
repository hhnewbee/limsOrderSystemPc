...
export default function BatchAddModal({ open, onCancel, onAdd, needBioinformaticsAnalysis }) {
    ...
    form.validateFields().then(values => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        newRows.push({
          ...
          // 仅当需要生信分析时填充这些字段
          analysisName: needBioinformaticsAnalysis ? finalAnalysisName : '',
          groupName: needBioinformaticsAnalysis ? groupName : '',
  ...
  return (
    <Modal
      title={needBioinformaticsAnalysis ? "新增实验分组 (批量)" : "批量添加样本"}
      open={open}
      onOk={handleOk}
      onCancel={onCancel}
      width={520}
